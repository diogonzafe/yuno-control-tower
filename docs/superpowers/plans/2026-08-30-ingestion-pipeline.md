...
Terminal 2 — hand-craft one event and push it onto the stream with `redis-cli` (or any Redis client pointed at the `REDIS_URL` from `.env`). Note `createdAt` is the epoch, **not** today: this writes to (and the cleanup below deletes from) the shared production-shape database, so the bucket must be a minute no real or demo transaction can ever occupy — never re-run this snippet with a present-day timestamp.
```bash
redis-cli -u "$REDIS_URL" XADD stream:transactions '*' payload '{"transactionId":"11111111-1111-4111-8111-111111111111","merchantOrderId":"manual-check","merchantId":"BR_STORE_01","providerId":"adyen","country":"BR","paymentMethod":"CARD","currency":"BRL","amountMinor":1000,"fxRate":5,"fxRateDate":"2026-08-30","fxSource":"MOCK","amountUsdMinor":200,"status":"SUCCESS","declineCode":null,"rawDeclineCode":null,"cardBrand":"visa","cardType":"credit","cardBin":"411111","issuerId":"itau","token":null,"latencyMs":null,"createdAt":"1970-01-01T00:00:00.000Z"}'
```
...
psql "$DATABASE_URL" -c "select bucket, merchant_id, attempts, approved from rollup_minute where merchant_id = 'BR_STORE_01' and bucket = '1970-01-01 00:00:00+00';"
```
...
psql "$DATABASE_URL" -c "delete from rollup_minute where merchant_id = 'BR_STORE_01' and bucket = '1970-01-01 00:00:00+00';"
```
