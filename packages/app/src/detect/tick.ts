...
function retry(c: Candidate, input: TickInput, z: number): Candidate | EvidenceGap { const from = minusMinutes(input.bucket, THIN_CELL_WINDOW_MIN - 1); const agg = aggregate([...input.history.filter((r) => r.bucket >= from && r.bucket < input.bucket), ...input.windowRows].filter((r) => matchesFilter(r, c.dimensions))); const result = evaluate(agg.approved, agg.attempts, c.expectedRate, c.deltaPp, MIN_VOLUME, z); return result.state === "MATERIAL_DROP" && agg.attempts >= MIN_VOLUME ? { ...c, ci: result.ci, observedRate: agg.rate ?? 0, attempts: agg.attempts, approved: agg.approved, windowUsed: "5m" } : gap(c, input.bucket, agg.attempts); }
export function runDetectionTick(input: TickInput): TickOutput {
  ...
  // Every entry below is one hypothesis, and they are all tested against the
  // same window. Correcting for how many there are is spec.md §8 question 3:
  // at a fixed 95%, 69 slices a minute — near 99k tests a day — confirm drops on
  // healthy cells by arithmetic, which is what BR_STORE_02 kept doing on issuer
  // slices carrying 11-24 attempts (family-wise.test.ts). The verdicts the
  // sweeps computed on the way here are recomputed with that z; they were
  // decided one test at a time and this is the only place that knows the count.
  const tested = [
  ...
  const z = familyWiseZ(tested.length);
  const raw = dedupe(tested.map((c) => ({ ...c, ...evaluate(c.approved, c.attempts, c.expectedRate, c.deltaPp, MIN_VOLUME, z) })));
  const candidates: Candidate[] = [], gaps: EvidenceGap[] = [];
  ...
  for (const c of raw) { if (c.state === "INSUFFICIENT_EVIDENCE") gaps.push(gap(c, input.bucket)); else if (c.state !== "MATERIAL_DROP" || c.attempts >= MIN_VOLUME) candidates.push(c); else { const result = retry(c, input, z); if ("reason" in result) gaps.push(result); else candidates.push(result); } }
